...
enum TaskManager {instance;
    ...
    public boolean updateNodeTaskStatus(String parentTaskId, String nodeTaskId, NodeTaskStatus nodeTaskStatus) {
        return updateNodeTaskStatus(parentTaskId, nodeTaskId, nodeTaskStatus, false);
    }
    ...
    public boolean updateNodeTaskStatus(String parentTaskId, String nodeTaskId, NodeTaskStatus nodeTaskStatus,
        ...
        if (nodeTask == null) {
            throw new RuntimeException(
                    "No nodeTask(parentTaskId: " + parentTaskId + ", nodeTaskId: " + nodeTaskId + ")");
        }
        if (nodeTask.getNodeTaskStatus() == NodeTaskStatus.success && skipSucceed) {
            nodeTask.setNodeTaskStatus(NodeTaskStatus.success);
        } else {
            if (nodeTask.getNodeTaskStatus() == NodeTaskStatus.fail) {
                nodeTask.setType("retry");
            }
    ...
    public boolean canNodeTaskSchedule(String parentTaskId, String nodeTaskId) {
        ...
        if (ObjectUtils.isEmpty(nodeTask)) {
            throw new RuntimeException(
                    "parentTask has finish [or] any nodeTask exception,parentTaskId: " + parentTaskId);
        }

        if (CollectionUtils.isEmpty(nodeTask.getDependencies())) {
            return true;
        }

        // 判断依赖NodeTask是否执行完成
        for (Object dependTaskId : nodeTask.getDependencies()) {
    ...
    public List<NodeTask> nodeTasksToBeScheduled(String parentTaskId) {
        ...
        for (NodeTask nodeTask : parentTask.getNodeTasks().values()) {
            if (nodeTask.getNodeTaskStatus() == NodeTaskStatus.init
                    || nodeTask.getNodeTaskStatus() == NodeTaskStatus.fail) {
    ...
    public void updateParentTaskStatus(String parentTaskId, NodeTaskStatus status) {
        ParentTask parentTask = getParentTask(parentTaskId);
        parentTask.setParentStatus(status);
    }}
